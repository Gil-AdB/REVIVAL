#!/usr/bin/env bash
# One number out of the v4 phase-3 census, for a ledger recheck.
#   tools/v4_p3_num.sh <key> [extra DEMO flags...]
# <key> is any key of tools/v4_census.py --p3 (plateau_emr_p50, plateau_over_tol,
# plateau_core_emr_p50, nodes_pinned, chart_maxdev_deg, ...).  Renders greets
# cam A once with --greets_displace_v4 --v4_census and prints the value.
set -u
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
KEY="$1"; shift
OUT="${TMPDIR:-/tmp}/v4_p3_num"
...
rm -f "$OUT"/*.ppm
python3 "$ROOT/tools/v4_census.py" --p3 "$OUT/log.txt" | awk -v k="$KEY" '$1==k{print $2}'
