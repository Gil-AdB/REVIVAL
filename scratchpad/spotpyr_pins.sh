...
# spotpyr worktree's Runtime/ (DEMO chdirs to its OWN directory, so this is a
# one-asset-tree differential).
#   usage: spotpyr_pins.sh <binary-name-in-Runtime> <tag> [runs]
set -u
WT=/Users/gil-ad/work/rev-spotpyr
BIN=$1; TAG=$2; RUNS=${3:-3}
OUT=${PIN_OUT:-/tmp/spotpyr/pins}/$TAG
