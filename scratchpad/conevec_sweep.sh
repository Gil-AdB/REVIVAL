#!/bin/zsh
# City t-sweep of the cone pass, scalar arm vs 8-wide solve, interleaved per
...
for p in $POSES; do
  for r in $(seq 1 $ROUNDS); do
    if (( r % 2 )); then run $p $r SCA --no-vol_cone_solve_vec; run $p $r VEC --vol_cone_solve_vec
    else                 run $p $r VEC --vol_cone_solve_vec; run $p $r SCA --no-vol_cone_solve_vec; fi
  done
