...
# Every pin was recorded at 1920x1080. Without these the rows render at Runtime/rev.cfg
# resolution - and the binary finds rev.cfg from the REAL executable path (REV.CPP:526),
# so a locally edited cfg fails all 12 rows with no pixel change in the tree (2026-09-01).
RES="--force_xres=1920 --force_yres=1080"   # zsh: expand with ${=RES}, an unquoted $RES is ONE argument
cd "$RT" || exit 1
PASS=0; FAIL=0
...
FDS_CITY_ENV_PIXEL=1 "$BIN" --snapshot=city@t=1961 --out="$d" --deferred ${=RES} >/dev/null 2>&1
...
for t in 1961:4cb8d2ca68b72f8a24627f42077eef25 2400:f473fe2b2658fa0c1c290e1acf8265b9 400:d3374de6a0840a6927e00eb54b48b359; do
  ...
  "$BIN" --snapshot=city@t=$tt --out="$d" --env_live_water --deferred --city_env_pixel ${=RES} >/dev/null 2>&1
  chk "city-acc t=$tt" "$ex " "$(h $d)"
done
d="$OUT/g1588"; mkdir -p "$d"
FDS_GREETS_CAM="-0.616376519,2.79000092,-24.4848595,0.164780021,-0.314234257,0.93493551" "$BIN" --snapshot=greets@t=1588 --out="$d" --deferred --hdr --glass-refract=1 --glass-test --xpar-peel-passes=4 --profiler=0 --no-env_refl ${=RES} >/dev/null 2>&1
...
for t in 5743:08f0741fb8c5d324a2303fb03ed23437 2845:84e18552fa079409a3da12c4b922bf77 6097:0cc05784dd3e9e448e3ad06df77180bc 6133:456eac7dc6d6b2135f0e975e11b3a4fe; do
  ...
  "$BIN" --snapshot=greets@t=$tt --out="$d" --deferred --hdr --hdr-linear --texture-filter=2 --ssao --ssao-gtao --greets-displace --profiler=0 ${=RES} >/dev/null 2>&1
  chk "greets-acc t=$tt" "$ex " "$(h $d)"
done
d="$OUT/fount"; mkdir -p "$d"
"$BIN" --snapshot=fountain@t=2500 --out="$d" --deferred --hdr --glass-refract=1 --glass-test --profiler=0 ${=RES} >/dev/null 2>&1
chk fountain "0222f90363dabb38ab52eedc286bdb65 " "$(h $d)"
d="$OUT/chase"; mkdir -p "$d"
"$BIN" --snapshot=chase@t=100,400,800,1200,1600 --out="$d" --deferred ${=RES} >/dev/null 2>&1
...
"$BIN" --cinematic --deferred --snapshot=chase@t=800,1600 --out="$d" ${=RES} >/dev/null 2>&1
