#!/bin/zsh
# Interleaved ABBA A/B of the leftover per-lane loops (--vol_cone_lane_vec) against
# the scalar arm (--no-vol_cone_lane_vec), one binary, one asset tree.
# Reports per-arm min-over-rounds of cones wall_min / Ginstr/f / Gcyc/f / IPC
# and the same for renderFrame.  usage: lanevec_ab.sh [rounds] [t] [iters]
set -u
ROUNDS=${1:-6}; POSE=${2:-1961}; ITERS=${3:-6}; SCENE=${4:-city}
OUT=${AB_OUT:-/tmp/lanevec_ab}
BIN=${BIN:-./DEMO}
...
for r in $(seq 1 $ROUNDS); do
  if (( r % 2 )); then run $r LVEC --vol_cone_lane_vec; run $r LSCA --no-vol_cone_lane_vec
  else                 run $r LSCA --no-vol_cone_lane_vec; run $r LVEC --vol_cone_lane_vec; fi
...
for name in ("renderFrame", "cones", "DeferredLighting-call", "fastfog", "gbuffer", "TBR-render"):
    ...
    a = rows[name].get("LSCA"); b = rows[name].get("LVEC")
    if a and b:
