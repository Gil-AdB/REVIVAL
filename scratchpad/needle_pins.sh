#!/usr/bin/env bash
# 2026-08-16x gate battery: the 11 pin recipes (15 hashes) in ONE worktree /
# ONE asset tree. Adapted verbatim from 2026-08-16r's xform_pins.sh — the ONLY
# place the greets acceptance-pose recipe was ever written down — plus the
# greets t=1588 pin recipe it omitted.
#   usage: needle_pins.sh <binA> [binB ...]      EXTRA="--needle_cull" to arm it
set -u
...
hash_run() {  # $1=bin  $2=tag  rest=argv
  ...
  ./"$b" "$@" $EXTRA --out="$d" >/dev/null 2>&1
  printf '%-12s %-22s %s\n' "$b" "$tag" "$(md5 -q "$d"/*.ppm 2>/dev/null | tr '\n' ' ')"
  rm -rf "$d"
}
GCAM_1588="-0.616376519,2.79000092,-24.4848595,0.164780021,-0.314234257,0.93493551"
...
for b in "$@"; do
 for i in $(seq 1 "$N"); do
  ...
  # chase: recorded recipe is VERBATIM without --profiler=0.
  ...
  FDS_GREETS_CAM="$GCAM_1588" \
  hash_run "$b" greets-t1588      --snapshot=greets@t=1588 --deferred --hdr --glass-refract=1 --glass-test --xpar-peel-passes=4 --profiler=0 --no-env_refl
  # shellcheck disable=SC2086
  hash_run "$b" greets-acc-t5743  --snapshot=greets@t=5743 $GACC
  hash_run "$b" greets-acc-t2845  --snapshot=greets@t=2845 $GACC
  hash_run "$b" greets-acc-t6097  --snapshot=greets@t=6097 $GACC
  hash_run "$b" greets-acc-t6133  --snapshot=greets@t=6133 $GACC
 done
done
