...
> 592 commits here, 45 on soa-vertex. Merged with `origin/fog-wt` as the base;
> 11 files conflicted, 40 hunks. **What soa-vertex brought in:** the DOS
> Open-Watcom revival build (`dos-build/`, 14 commits, zero overlap with this
> line), the RenderContext step-4 poison campaign (9 TUs `#pragma GCC poison`
> the render-target globals), the parallel mirror-RTT fan, the `screen_text`
> HUD kill-switch, and the chase water dark-band fix (9902349 — this line had
> already integrated it, so it merged as comment-only).
>
> **Resolution rule where the two collided: fog-wt's semantics expressed through
> soa-vertex's ctx.** The 12x8 tile grids, the SIMD fog composite, `TailProf`
> rows, per-surface `WaterProcMode`, glass refraction and the env/cam debug
> blocks are all this line's; they now read `ctx.xres/yres/vpage/zpage16/
> cntrEX/fovX/...` instead of the globals. Every poisoned-identifier site the
> compiler flagged was converted (`DeferredFastFog.cpp` and
> `DeferredSurfaceKernel.cpp` — the other seven poisoned TUs auto-merged
> clean), including `fillFallsBackHere`'s poisoned PARAMETER names,
> `glassBandSnapshotBegin` (now takes the ctx — an offscreen bake composites
> glass into its own surface at its own size) and `Render_DeferredSkyGradient`
> (now takes the ctx). `ctx.hdrBuf` (this line) is kept as the HDR target, but
> is fed from `Hdr_BufData()` and read through `Hdr_Active()` so a thread-local
> HDR override still wins — that is what soa-vertex's parallel bakes need.
>
> **Silent trap caught:** this line's `GreetsMirror.cpp` never set `dctx.Sc`,
> and the merged kernel returns early on `!Sc`. Unfixed, every RTT slot would
