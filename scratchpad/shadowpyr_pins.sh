#!/bin/zsh
# Pin gate: canonical snapshot recipes for a NAMED binary in the shadowpyr
# worktree's Runtime/ (DEMO chdirs to its own directory).
#   usage: pins.sh /path/to/DEMO tag [runs]
set -u
WT=/Users/gil-ad/work/rev-shadowpyr
BIN=$1; TAG=$2; RUNS=${3:-3}
OUT=${PIN_OUT:-/tmp/shadowpyr/pins}/$TAG
mkdir -p $OUT
