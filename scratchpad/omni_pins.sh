#!/bin/zsh
# Pin gate for the omni-loop campaign. Runs the canonical snapshot recipes for
# a NAMED binary in this worktree's Runtime/ (DEMO chdirs to its own directory,
# so the binary must be copied there — running the worktree binary from another
# Runtime/ renders the OTHER tree's assets).
#
#   usage: omni_pins.sh /path/to/DEMO tag [runs]
set -u
WT=/Users/gil-ad/work/rev-omniloop
BIN=$1; TAG=$2; RUNS=${3:-2}
OUT=${PIN_OUT:-/tmp/omniloop/pins}/$TAG
mkdir -p $OUT
