...
// Takes the pass's target dims/pointers (ctx) rather than the engine globals —
// an offscreen bake composites glass into ITS OWN surface at ITS OWN size.
static void glassBandSnapshotBegin(const DeferredLightingCtx &ctx,
                                   int strip_y, int strip_h) {
	const int xres = ctx.xres, yres = ctx.yres;
	const int y0 = strip_y < 0 ? 0 : strip_y;
	const int y1 = std::min(strip_y + strip_h, yres);
	...
	if (ctx.vpage) {
		...
		std::memcpy(t_glassBandLdr.data(),
		            reinterpret_cast<const uint32_t*>(ctx.vpage) + size_t(y0) * size_t(xres),
			...
			if (checker || quarter) {
				...
				if (drop) {
					...
					if (envTabG && (checkerEnvFull || checkerEdgeFull)) {
						const uint32_t m32e = gb.txtr[size_t(py) * ctx.xres + px];
					...
					if (!keep && checkerEdgeFull && !envHere)
						keep = fillFallsBackHere(gb, ctx.zpage16, ctx.xres, ctx.yres,
						                         px, py, quarter,
	...
	for (int py = y1; py < y2; ++py) {
		for (int px = x1; px < x2; ++px) {
			...
			if (fds::Hdr_Active() && hdrBufReady) {   // hdrBufReady: skip in the mirror RTT
				fds::hdrf* h = ctx.hdrBuf + i * 4;   // write target stays at this pixel
                             ...
                             int strip_y, int strip_h, bool bandSnapshot)
{
	...
	if (glassClump) glassBandSnapshotBegin(dctx, strip_y, strip_h);
                                              ...
                                              int x1, int y1, int x2, int y2)
{
	...
	const bool checkerEdgeFullG = (checker || quarter)
	    && fds::FeatureFlags::deferred_checker_edge_full()
	    && !deferredLightingOuterVecEnabled(ctx.Sc);
...
void Render_DeferredLighting(DeferredLightingCtx &ctx, const DeferredOverride *ov) {
	...
	ctx.hdrBuf = (ov && ov->hdr)
	               ? ov->hdr
	               : (fds::Hdr_WritableFor(ctx.xres, ctx.yres) ? fds::Hdr_BufData()
	                                                   : nullptr);
