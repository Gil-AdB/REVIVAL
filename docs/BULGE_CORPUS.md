...
## 2026-08-29 — his verdict on the tears fix and on shoulder_plateau

Three arms flown on the rev-dispfix binary (base flags as above plus
`--greets-displace`); his words verbatim, relayed by the coordinator:

> "C looks good. B has holes (makes sense). A also okay"

and then, as the decision:

> "go for shoulder_plateau"

| # | state / arm | recipe (on top of the base flags) | label | his words |
|---|---|---|---|---|
| 10 | **A** — the tears default before 2026-08-29 | `--greets-displace` (tsplit + border_weld ON, shoulder_plateau OFF) | **OK** | "A also okay" — the first arm in this campaign he has called okay rather than broken. |
| 11 | **B** — the pre-tears-round arm | `--greets-displace --no-greets_displace_tsplit --no-greets_displace_border_weld` | **BROKEN (holes)** | "B has holes (makes sense)" — and he is right: this arm measures 8560 battery holes against A's 5337, and 5166 at his own H6194 against A's 5084. His eye and the ground-truth hole detector ORDER THE SAME WAY, which is the first independent confirmation the detector has had. |
| 12 | **C** — A plus the shoulder-plateau relief | `--greets-displace --greets_displace_shoulder_plateau` | **OK, BEST** | "C looks good" → "go for shoulder_plateau". C is now the DEFAULT (FeatureFlags.def, 2026-08-29). |

Two things this settles for the record:
- The ray-space PROTRUSION fraction at his own marks is **not** a look metric.
  C scores WORSE on it than A (rect 13.4 → 18.8 %, ellipse 6.7 → 8.5 %) and he
  picked C. What tracked his eye is the NORMAL-space plateau bias (−0.036 →
  −0.018 u on every wall plane) and p90 |dz| (0.393/0.503 → 0.279/0.358).
  Ray-space protrusion against a mip-2 blurred reference reads a crisper
  shoulder as a defect; his eye reads it as stone. Use it as a regression tripwire,
  never as a verdict.
- "makes sense" on B is him confirming the hole class is real and visible, not
  an artifact of the detector.

Also new, and NOT yet labeled by him: pose **H6194** (t=6194,
`FDS_GREETS_CAM="22.4811096,5.24028063,-63.2136497,-0.996247888,-0.0673760772,-0.0543192849"`),
which he supplied with "in any case, see this pose". It is the worst hole pose
in the campaign (5084 px on arm A) and it is a level-boundary T-junction crack —
see SESSION_STATE 2026-08-29b. He has given no verdict on it; absence of
complaint is never an OK label.
